...
struct command {
  ...
  /* dynamically generated help; if non-nullptr,
...
static struct command commands[] = {
    ...
    {"help", ALLOW_INFO,
     ...
     N_("With no arguments gives some introductory help. "
        ...
        "and help is given for that command or option. For options, the "
        "help "
        "information includes the current and default values for that "
        "option. "
    ...
    {"quit", ALLOW_HACK,
     // no translatable parameters
     SYN_ORIG_("quit"), N_("Quit the game and shutdown the server."),
     nullptr, nullptr, CMD_ECHO_ALL, VCF_NONE, 0},
    {"cut", ALLOW_CTRL,
     // TRANS: translate text between <> only
     ...
     N_("Cut specified client's connection to the server, removing that "
        "client "
        "from the game. If the game has not yet started that client's "
        "player "
        "is removed from the game, otherwise there is no effect on the "
        "player. "
        "Note that this command now takes connection names, not player "
        "names."),
    ...
    {"explain", ALLOW_INFO,
     ...
     N_("The 'explain' command gives a subset of the functionality of "
        "'help', "
        "and is included for backward compatibility. With no arguments it "
        "gives a list of options (like 'help options'), and with an "
        "argument "
        "it gives help for a particular option (like 'help "
        "<option-name>')."),
     nullptr, CMD_ECHO_NONE, VCF_NONE, 0},
    {"show", ALLOW_INFO,
     // TRANS: translate text between <> only
     ...
     N_("With no arguments, shows vital server options (or available "
        "options, "
        ...
        "options with this level. With \"changed\", it shows only the "
        "options "
        "which have been modified, while with \"locked\" all settings "
        "locked "
    ...
    {"vote", ALLOW_BASIC,
     ...
     N_("A player with basic level access issuing a control level command "
        ...
        "gives your vote. If you do not add a vote number, your vote "
        "applies "
    ...
    {"set", ALLOW_CTRL,
     ...
     N_("Set an option on the server. The syntax and legal values depend "
        ...
        "values; "
        "separate these with |, for instance, '/set topology wrapx|iso'. "
        "For "
    ...
    {"team", ALLOW_CTRL,
     ...
     N_("A team is a group of players that start out allied, with shared "
        ...
        "team a player is a member of. Use \"\" if names contain "
        "whitespace."),
     nullptr, CMD_ECHO_ALL, VCF_NONE, 50},
    {"rulesetdir", ALLOW_CTRL,
     // TRANS: translate text between <> only
     N_("rulesetdir <directory>"),
     N_("Choose new ruleset directory or modpack."), nullptr, nullptr,
    ...
    {"metamessage", ALLOW_CTRL,
     ...
     nullptr, CMD_ECHO_ADMINS, VCF_NONE, 50},
    {"metapatches", ALLOW_HACK,
     // TRANS: translate text between <> only
     N_("metapatches <meta-line>"), N_("Set metaserver patches line."),
     nullptr, nullptr, CMD_ECHO_ADMINS, VCF_NONE, 0},
    {"metaconnection", ALLOW_ADMIN,
     ...
     N_("'metaconnection ?' reports on the status of the connection to "
        "metaserver. "
        "'metaconnection down' or 'metac d' brings the metaserver "
        "connection down. "
        "'metaconnection up' or 'metac u' brings the metaserver connection "
        "up. "
        "'metaconnection persistent' or 'metac p' is like 'up', but keeps "
        "trying after failures. "),
     nullptr, CMD_ECHO_ADMINS, VCF_NONE, 0},
    {"metaserver", ALLOW_ADMIN,
     // TRANS: translate text between <> only
     N_("metaserver <address>"),
     N_("Set address (URL) for metaserver to report to."), nullptr, nullptr,
    ...
    {"take", ALLOW_INFO,
     ...
     N_("Only the console and connections with cmdlevel 'hack' can force "
        ...
        "these, "
        "only the <player-name> argument is allowed. If '-' is given for "
        "the "
        "player name and the connection does not already control a player, "
        "one "
    ...
    {"observe", ALLOW_INFO,
     ...
     N_("Only the console and connections with cmdlevel 'hack' can force "
        ...
        "no "
        "player-name or the connection uses no arguments, then the "
        "connection "
    ...
    {"create", ALLOW_CTRL,
     ...
     N_("With the 'create' command a new player with the given name is "
        ...
        "from its type.\n"
        "The 'ai type' parameter can be used to select which AI module will "
        "be "
        "used for the created player. This requires that the respective "
        "module "
        "has been loaded or built in to the server.\n"
        "If the game has already started, the new player will have no units "
        "or "
        "cities; also, if no free player slots are available, the slot of a "
        "dead player can be reused (removing all record of that player from "
        "the "
        "running game)."),
     nullptr, CMD_ECHO_ALL, VCF_NONE, 50},
    {"away", ALLOW_BASIC,
     // no translatable parameters
     SYN_ORIG_("away"),
     N_("Set yourself in away mode. The AI will watch your back."), nullptr,
    ...
    {"handicapped", ALLOW_CTRL,
     ...
     N_("Set one or all AI players to 'Handicapped'."), nullptr,
     ai_level_help, CMD_ECHO_ALL, VCF_NONE, 50},
    {"novice", ALLOW_CTRL,
     // TRANS: translate text between <> only
     N_("novice\n"
        "novice <player-name>"),
     // TRANS: translate 'Novice' as AI skill level
     N_("Set one or all AI players to 'Novice'."), nullptr, ai_level_help,
     CMD_ECHO_ALL, VCF_NONE, 50},
    {"easy", ALLOW_CTRL,
     // TRANS: translate text between <> only
     N_("easy\n"
     ...
     N_("Set one or all AI players to 'Easy'."), nullptr, ai_level_help,
     CMD_ECHO_ALL, VCF_NONE, 50},
    {"normal", ALLOW_CTRL,
     // TRANS: translate text between <> only
     N_("normal\n"
     ...
     N_("Set one or all AI players to 'Normal'."), nullptr, ai_level_help,
     CMD_ECHO_ALL, VCF_NONE, 50},
    {"hard", ALLOW_CTRL,
     // TRANS: translate text between <> only
     N_("hard\n"
     ...
     N_("Set one or all AI players to 'Hard'."), nullptr, ai_level_help,
     CMD_ECHO_ALL, VCF_NONE, 50},
    {"cheating", ALLOW_CTRL,
     // TRANS: translate text between <> only
     N_("cheating\n"
     ...
     N_("Set one or all AI players to 'Cheating'."), nullptr, ai_level_help,
     CMD_ECHO_ALL, VCF_NONE, 50},
#ifdef FREECIV_DEBUG
    {"experimental", ALLOW_CTRL,
     ...
     N_("Set one or all AI players to 'Experimental'."), nullptr,
     ai_level_help, CMD_ECHO_ALL, VCF_NONE, 50},
#endif // FREECIV_DEBUG
    {"cmdlevel", ALLOW_ADMIN,
     ...
     N_("The command access level controls which server commands are "
        "available "
        ...
        "clients. "
        "If 'first come' is specified, the 'first come' level is set; it "
        "will be "
        "granted to the first client to connect, or if there are "
        "connections "
    ...
    {"playercolor", ALLOW_ADMIN,
     ...
     N_("This command sets the color of a specific player, overriding any "
        "color "
        "assigned according to the 'plrcolormode' setting.\n"
        "The color is defined using hexadecimal notation (hex) for the "
        "combination of Red, Green, and Blue color components (RGB), "
        "similarly "
        ...
        "'plrcolormode' setting is set to 'PLR_SET'; a player's color can "
        "be "
        "unset again by specifying 'reset'.\n"
        "Once the game has started and colors have been assigned, this "
        "command "
    ...
    {"endgame", ALLOW_ADMIN,
     // no translatable parameters
     SYN_ORIG_("endgame"), N_("End the game immediately in a draw."),
     nullptr, nullptr, CMD_ECHO_ALL, VCF_NONE, 0},
    ...
    {"scensave", ALLOW_ADMIN,
     ...
     N_("Save the current game to file <file-name> as scenario. If no "
        "file-name "
        "argument is given saves to \"<auto-save name "
        "prefix><year>m.sav[.gz]\". "
    ...
    {"read", ALLOW_CTRL,
     // TRANS: translate text between <> only
     N_("read <file-name>"), N_("Process server commands from file."),
     nullptr, nullptr, CMD_ECHO_ADMINS, VCF_NONE, 50},
    {"write", ALLOW_HACK,
     // TRANS: translate text between <> only
     N_("write <file-name>"),
     N_("Write current settings as server commands to file."), nullptr,
     nullptr, CMD_ECHO_ADMINS, VCF_NONE, 0},
    {"reset", ALLOW_CTRL,
     // no translatable parameters
    ...
    {"kick", ALLOW_CTRL,
     ...
     nullptr, CMD_ECHO_ADMINS, VCF_NOPASSALONE, 50},
    {"delegate", ALLOW_BASIC,
     ...
     N_("Delegation allows a user to nominate another user who can "
        "temporarily "
        ...
        "been "
        "delegated to you. (Behaves like 'take', except that the "
        "'allowtake' "
        "restrictions are not enforced.)\n"
        "'delegate restore': relinquish control of a delegated player "
        "(opposite "
        "of 'delegate take') and restore your previous view, if any. (This "
        "also "
    ...
    {"fcdb", ALLOW_ADMIN,
     ...
     N_("The argument 'reload' causes the database script file to be "
        "re-read "
    ...
    {"rfcstyle", ALLOW_HACK,
     // no translatable parameters
     SYN_ORIG_("rfcstyle"),
     N_("Switch server output between 'RFC-style' and normal style."),
     nullptr, nullptr, CMD_ECHO_ADMINS, VCF_NONE, 0},
    {"serverid", ALLOW_INFO,
     // no translatable parameters
...
{
  fc_assert_ret_val(i >= 0 && i < CMD_NUM, nullptr);
  return &commands[i];
}
...
char *command_extra_help(const struct command *pcommand)
{
  if (pcommand->extra_help_func) {
    fc_assert(pcommand->extra_help == nullptr);
