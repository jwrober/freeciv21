...
{
  if (nullptr == arrange_workers_queue) {
...
{
  nation_city_list_iterate(nation_cities(nation_of_player(pplayer)), pncity)
  {
                          ...
                          char *error_buf, size_t bufsz)
{
  ...
  if ((CNM_GLOBAL_UNIQUE == game.server.allowed_city_names
    ...
    if (error_buf) {
      fc_snprintf(error_buf, bufsz, _("A city called %s already exists."),
                  cityname);
    }
    return false;
  }
...
const char *city_name_suggestion(struct player *pplayer, struct tile *ptile)
{
  ...
  // Not found in rulesets, make a default name.
  {
    ...
    for (i = 1; i <= map_num_tiles(); i++) {
      ...
      if (nullptr == game_city_by_name(tempname)) {
        return tempname;
      }
    }
                          ...
                          bool rehome, bool verbose)
{
  ...
  fc_assert_ret_msg(!punit->server.dying,
                    "Tried to transfer the dying unit %d.", punit->id);
  ...
  if (from_player == to_player) {
    ...
    if (verbose) {
      notify_player(from_player, unit_tile(punit), E_UNIT_RELOCATED,
                    ...
                    unit_link(punit), city_link(tocity));
    }
  } else {
      ...
      if (utype_has_flag(unit_type_get(punit), UTYF_GAMELOSS)) {
        ...
        if (verbose) {
          notify_player(
              ...
              nation_plural_for_player(to_player), unit_tile_link(punit));
        }
    ...
    if (in_city) {
      ...
      if (verbose) {
        notify_player(from_player, unit_tile(punit), E_UNIT_RELOCATED,
                      ...
                      unit_link(punit), city_link(in_city),
      ...
      if (verbose) {
        notify_player(
            ...
            nation_adjective_for_player(from_player), unit_tile_link(punit),
            nation_adjective_for_player(to_player), city_link(tocity));
      }
      wipe_unit(punit, ULR_CITY_LOST, nullptr);
      return;
    }
  ...
  if (pplayer != pvictim) {
    unit_list_iterate_safe((ptile)->units, vunit)
    {
      ...
      if (unit_owner(vunit) == pvictim) {
         ...
         * city transfer. */
        bool homeless =
            (vunit->homecity == 0) && !unit_list_search(units, vunit);
  ...
  unit_list_iterate_safe(units, vunit)
  {
    ...
    if (new_home_city && new_home_city != exclude_city
      ...
      transfer_unit(vunit, new_home_city, true, verbose);
                ...
                || real_map_distance(unit_tile(vunit), ptile)
                       <= kill_outside)
               && saved_id) {
...
/**
   ...
   'only_player'     if set and 'pplayer' is not nullptr only cities of this
                     player are returned.
   'only_enemy'      if set and 'pplayer' is not nullptr only cities of
 players which are at war with 'pplayer' are returned. 'pclass'          if
 set, and 'pclass' is not nullptr only cities that have adjacent native
...
/**
   ...
   always palace) with game.server.razechance% chance, barbarians destroy
 more set the city's shield stock to 0
 */
static void raze_city(struct city *pcity)
{
  ...
  city_built_iterate(pcity, pimprove)
  {
    ...
    if (is_improvement(pimprove) && (fc_rand(100) < razechance)) {
      /* FIXME: Should maybe have conquering unit instead of nullptr as
       * destoryer */
...
static void reestablish_city_trade_routes(struct city *pcity)
{
  trade_routes_iterate_safe(pcity, proute)
  {
     ...
     * Do not announce removal as we might restore the route immediately
     * below */
                                     ...
                                     bv_imprs *had_small_wonders)
{
  ...
  improvement_iterate(pimprove)
  {
    if (improvement_has_flag(pimprove, IF_SAVE_SMALL_WONDER)
      ...
      fc_assert_action(nullptr == city_from_small_wonder(pplayer, pimprove),
                       continue);
                   ...
                   bool resolve_stack, bool raze, bool build_free)
{
  ...
  const citizens old_taker_content_citizens =
      player_content_citizens(ptaker);
  const citizens old_giver_content_citizens =
  ...
  bv_player *could_see_unit =
      (units_num > 0 ? new bv_player[units_num] : nullptr);
  ...
  transfer_city_units(ptaker, pgiver, old_city_units, pcity, nullptr,
  ...
  if (city_remains) {
     ...
     * lose vision to it. When we later send info to everybody seeing the
     * city, (s)he may not be included. */
  ...
  if (had_great_wonders) {
    ...
    if (city_remains) {
      ...
      cities_iterate(acity) { city_refresh_queue_add(acity); }
      cities_iterate_end;
    }
  }
                 ...
                 const char *name, struct player *nationality)
{
  ...
  notify_player(pplayer, ptile, E_CITY_BUILD, ftc_server,
                _("You have founded %s."), city_link(pcity));
...
void remove_city(struct city *pcity)
{
  ...
  unit_list_iterate_safe(pcity->units_supported, punit)
  {
    ...
    if (new_home_city && new_home_city != pcity
        && city_owner(new_home_city) == powner && !punit->server.dying) {
        ...
        if (adv_could_unit_move_to_tile(punit, tile1) == 1) {
          ...
          if (!can_unit_survive_at_tile(&(wld.map), punit, tile1)
              ...
              && is_action_enabled_unit_on_unit(ACTION_TRANSPORT_EMBARK,
                                                punit, ptrans)) {
            // "Transport Embark".
            moved = unit_perform_action(
                unit_owner(punit), punit->id, ptrans->id, 0, "",
      ...
      if (other_city != nullptr) {
        ...
        unit_list_iterate_safe(piter->units, punit)
        {
          ...
          if (!uclass_has_flag(pclass, UCF_BUILD_ANYWHERE)
            ...
            notify_player(
                unit_owner(punit), unit_tile(punit), E_UNIT_LOST_MISC,
                ftc_server,
                _("When %s was disbanded your %s in %s was trapped, "
                  "and it was therefore lost."),
                ctl, unit_tile_link(punit), city_link(other_city));
            wipe_unit(punit, ULR_CITY_LOST, nullptr);
          }
        }
        unit_list_iterate_safe_end;
...
bool unit_conquer_city(struct unit *punit, struct city *pcity)
{
  ...
  fc_assert_ret_val_msg(
      unit_can_take_over(punit)
          || utype_can_do_action(unit_type_get(punit), ACTION_PARADROP),
  ...
  */
  ...
  if (is_capital(pcity) && civil_war_possible(cplayer, true, true)
...
void send_all_known_cities(struct conn_list *dest)
{
  ...
  conn_list_iterate(dest, pconn)
  {
    ...
    {
      if (!pplayer || nullptr != map_get_player_site(ptile, pplayer)) {
...
/**
 ...
 info about a city to a player who thinks the tile contains another city. If
 you want to update the clients info of the tile you must use
                            ...
                            struct city *pcity, struct tile *ptile)
{
  ...
  struct traderoute_packet_list *routes = nullptr;
  ...
  if (powner && powner == pviewer) {
    ...
    if (!pviewer) { // observer
      if (pcity) {
        ...
        package_city(pcity, &packet, routes,
                     false); // should be dumb_city info?
        lsend_packet_city_info(dest, &packet, false);
        traderoute_packet_list_iterate(routes, route_packet)
        {
          lsend_packet_traderoute_info(dest, route_packet);
      ...
      if (map_is_known_and_seen(ptile, pviewer, V_MAIN)) {
        ...
        if (nullptr != map_get_player_site(ptile, pviewer)) {
          package_dumb_city(pviewer, ptile, &sc_pack);
          lsend_packet_city_short_info(dest, &sc_pack);
        }
      }
  ...
  if (game.info.citizen_nationality) {
    ...
    player_slots_iterate(pslot)
    {
      ...
      if (nationality != 0) {
        ...
        packet->nation_citizens[packet->nationalities_count] = nationality;
...
bool update_dumb_city(struct player *pplayer, struct city *pcity)
{
  ...
  } else if (pdcity->occupied == occupied && pdcity->walls == walls
             && pdcity->happy == happy && pdcity->unhappy == unhappy
                                         ...
                                         bool source_gone)
{
  ...
  if (plr1 == plr2) {
    if (source_gone) {
      ...
      notify_player(plr1, city_tile(pc1), E_CARAVAN_ACTION, ftc_server,
                    _("Trade route between %s and %s canceled."), city1_link,
                    city2_link);
    }
  } else {
    if (source_gone) {
      ...
      notify_player(plr1, city_tile(pc1), E_CARAVAN_ACTION, ftc_server,
                    ...
                    _("We canceled the trade route "
                      "from %s to %s city %s."),
                    city1_link, nation_adjective_for_player(plr2),
                    city2_link);
    }
  }
}
...
void city_illness_strike(struct city *pcity)
{
  notify_player(city_owner(pcity), city_tile(pcity), E_CITY_PLAGUE,
                ...
                city_link(pcity));
  city_reduce_size(pcity, 1, nullptr, "plague");
                         ...
                         struct universal *target, enum event_type event)
{
  ...
  if (VUT_IMPROVEMENT == pcity->production.kind
    ...
    notify_player(nullptr, city_tile(pcity), E_WONDER_STOPPED, ftc_server,
                  ...
                  city_production_name_translation(pcity), city_link(pcity));
  }
  ...
  notify_player(pplayer, city_tile(pcity), event, ftc_server,
                 ...
                 * " from the worklist". */
                _("%s is building %s%s."), city_link(pcity), name, source);
...
{
  city_list_iterate(pplayer->cities, pcity) { city_refresh_vision(pcity); }
  city_list_iterate_end;
}
...
bool city_map_update_radius_sq(struct city *pcity)
{
  ...
  if (city_tiles_old < city_tiles_new) {
                               ...
                               city_x, city_y)
    {
      struct tile *ptile = city_map_to_tile(
          city_tile(pcity), city_radius_sq_old, city_x, city_y);
  ...
  notify_player(
      city_owner(pcity), city_tile(pcity), E_CITY_RADIUS_SQ, ftc_server,
      _("The size of the city map of %s is %s."), city_name_get(pcity),
...
int city_production_buy_gold_cost(const struct city *pcity)
{
  ...
  case VUT_IMPROVEMENT:
    return impr_buy_gold_cost(pcity, pcity->production.value.building,
                              build);
  case VUT_UTYPE:
    return utype_buy_gold_cost(pcity, pcity->production.value.utype, build);
  default:
    break;
  };
