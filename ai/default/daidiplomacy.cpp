...
static void dai_incident_simple(struct player *receiver,
                                ...
                                enum casus_belli_range scope, int how_bad);
...
{
  return (player1->ai_common.love[player_index(player2)]
              > -(MAX_AI_LOVE * 4 / 10)
                                   ...
                                   bool *is_dangerous)
{
  ...
  *is_dangerous = false;
  ...
  if (research_invention_state(research_get(taker), tech_id) == TECH_KNOWN) {
    return 0;
  }
  ...
  players_iterate_alive(eplayer)
  {
    if (eplayer == giver || eplayer == taker
        || research_invention_state(research_get(eplayer), tech_id)
               == TECH_KNOWN) {
      continue;
    }
                             ...
                             const struct player *them)
{
  ...
  {
    if (aplayer != us && aplayer != them && pplayers_allied(them, aplayer)
                                ...
                                enum diplstate_type ds_after)
{
  ...
  case CLAUSE_ADVANCE:
    if (give) {
      ...
      if (game.info.tech_upkeep_style != TECH_UPKEEP_NONE) {
        ...
        int limit =
            MAX(1, player_tech_upkeep(pplayer) / research->techs_researched);
  ...
  case CLAUSE_CEASEFIRE:
    ...
    if (has_handicap(pplayer, H_AWAY)) {
      dai_diplo_notify(
          aplayer, _("*%s (AI)* In away mode AI can't sign such a treaty."),
          player_name(pplayer));
      worth = -BIG_NUMBER;
      break;
    }
    ...
    if (adip->is_allied_with_enemy && pclause->type != CLAUSE_CEASEFIRE) {
      dai_diplo_notify(aplayer,
                       _("*%s (AI)* I would like to see you keep your "
    ...
    if (pclause->type == CLAUSE_PEACE) {
      ...
      if (pplayer->ai_common.love[player_index(aplayer)]
          < MAX_AI_LOVE / 10) {
        dai_diplo_notify(aplayer,
                         _("*%s (AI)* I simply do not trust you with an "
                           "alliance yet, %s."),
                         player_name(pplayer), player_name(aplayer));
        worth = -BIG_NUMBER;
      }
      DIPLO_LOG(ait, LOG_DIPL, pplayer, aplayer, "ally clause worth %d",
                worth);
    } else {
      if (is_ai(pplayer) && is_ai(aplayer)
        ...
        if (turns < TURNS_BEFORE_TARGET) {
          ...
          DIPLO_LOG(ait, LOG_DIPL, pplayer, aplayer,
                    "ceasefire worth=%d love=%d "
                    "turns=%d",
                    worth, pplayer->ai_common.love[player_index(aplayer)],
                    turns);
        }
      }
    }
    ...
    DIPLO_LOG(ait, LOG_DIPL, pplayer, aplayer, "treaty clause worth %d",
              worth);
    break;
  ...
  case CLAUSE_MAP:
    if (!give || ds_after == DS_ALLIANCE) {
      ...
      // Don't like him? Don't give him!
      worth =
          MIN(pplayer->ai_common.love[player_index(aplayer)] * 10, worth);
      // Make maps from novice player cheap
      if (has_handicap(pplayer, H_DIPLOMACY)) {
        worth /= 6;
      }
  ...
  case CLAUSE_CITY: {
    struct city *offer =
        city_list_find_number(pclause->from->cities, pclause->value);
                         ...
                         struct player *aplayer, struct Treaty *ptreaty)
{
  ...
  if (total_balance >= 0) {
    ...
    if (pplayer != ptreaty->plr0) {
      dai_diplo_notify(
          aplayer, _("*%s (AI)* This deal was not very good for us, %s!"),
          player_name(pplayer), player_name(aplayer));
    }
                             ...
                             struct player *aplayer, struct Clause *pclause)
{
  ...
  case CLAUSE_CEASEFIRE:
    dai_diplo_notify(aplayer,
                     _("*%s (AI)* Agreed. No more hostilities, %s."),
                     player_name(pplayer), player_name(aplayer));
                         ...
                         struct player *aplayer, struct Treaty *ptreaty)
{
  ...
  if (total_balance > 0 && gift) {
    ...
    DIPLO_LOG(ait, LOG_DIPL2, pplayer, aplayer, "gift increased love by %d",
              i);
  }
                                 ...
                                 struct player *aplayer)
{
  ...
  if (is_ai(pplayer)
    ...
    if (has_handicap(pplayer, H_CEASEFIRE)) {
      ...
      if (dai_goldequiv_clause(ait, pplayer, aplayer, &clause, false,
                               DS_CEASEFIRE)
          > 0) {
        wants_ceasefire = true;
      }
                                   ...
                                   struct player *pplayer)
{
  ...
  players_iterate_alive(aplayer)
  {
    ...
    if (aplayer == pplayer || NEVER_MET(pplayer, aplayer)
        || players_on_same_team(pplayer, aplayer)) {
      continue;
    }
    war_desire[player_index(aplayer)] =
        dai_war_desire(ait, pplayer, aplayer);
  ...
  players_iterate_alive(aplayer)
  {
    ...
    if ((pplayers_non_attack(pplayer, aplayer)
        ...
        && adip->countdown == -1 && !adip->is_allied_with_enemy
        && !adip->at_war_with_ally && aplayer != best_target
      ...
      DIPLO_LOG(ait, LOG_DEBUG, pplayer, aplayer, "Increased love by %d",
                amount);
    } else if (WAR(pplayer, aplayer)) {
      amount -= ai->diplomacy.love_incr / 2;
      pplayer->ai_common.love[player_index(aplayer)] += amount;
      DIPLO_LOG(ait, LOG_DEBUG, pplayer, aplayer, "%d love lost to war",
                amount);
    ...
    if (amount != 0) {
      DIPLO_LOG(ait, LOG_DEBUG, pplayer, aplayer,
                "%d love lost due to units inside "
                "our borders",
                amount);
    }
                                  ...
                                  struct player *player2)
{
  ...
  {
    if (research_invention_state(presearch1, tech) == TECH_KNOWN) {
      if (research_invention_state(presearch2, tech) != TECH_KNOWN
          && research_invention_gettable(presearch2, tech,
                                         game.info.tech_trade_allow_holes)) {
  ...
  advance_index_iterate(A_FIRST, tech)
  {
    ...
    advance_index_iterate(A_FIRST, tech2)
    {
      ...
      if ((diff > 0 && player1->economic.gold >= diff)
          || (diff < 0 && player2->economic.gold >= -diff) || diff == 0) {
        dai_diplomacy_suggest(player1, player2, CLAUSE_ADVANCE, false,
                              tech2);
                      ...
                      struct player *aplayer)
{
  ...
  if (presearch != aresearch && players_on_same_team(pplayer, aplayer)) {
      ...
      if (research_invention_state(presearch, idx) != TECH_KNOWN
        ...
        dai_diplomacy_suggest(pplayer, aplayer, CLAUSE_ADVANCE, true, idx);
                 ...
                 && research_invention_gettable(
                     aresearch, idx, game.info.tech_trade_allow_holes)) {
  ...
  if (!gives_vision && shared_vision_is_safe(pplayer, aplayer)) {
    dai_diplomacy_suggest(pplayer, aplayer, CLAUSE_VISION, false, 0);
    gives_vision = true;
  }
  if (gives_vision && !gives_shared_vision(aplayer, pplayer)
      && (is_human(aplayer) || shared_vision_is_safe(aplayer, pplayer))) {
                          ...
                          struct player *target, enum war_reason reason)
{
  ...
  case DAI_WR_ALLIANCE:
    if (adip->at_war_with_ally) {
      ...
      // Ooops!
      DIPLO_LOG(ait, LOG_DIPL, pplayer, target,
  ...
  if (pplayer_can_cancel_treaty(pplayer, target) == DIPL_SENATE_BLOCKING) {
    handle_player_change_government(
        pplayer, game.info.government_during_revolution_id);
  }
                          ...
                          enum war_reason reason)
{
  ...
  DIPLO_LOG(ait, LOG_DIPL, pplayer, target, "countdown to war in %d",
            countdown);
  ...
  {
    if (!pplayers_allied(pplayer, ally) || ally == target
    ...
    case DAI_WR_SPACE:
      dai_diplo_notify(ally,
                       PL_("*%s (AI)* We will be launching an all-out war "
                           ...
                           "launch.",
                           countdown),
                       player_name(pplayer), player_name(target), countdown);
      dai_diplo_notify(
          ally,
      ...
      if (WAR(ally, target)) {
        ...
        dai_diplo_notify(
            ...
            PL_("*%s (AI)* We will honor our alliance with %s and "
                ...
                "do likewise.",
                countdown),
            player_name(pplayer), player_name(adip->at_war_with_ally),
            player_name(target), countdown);
      } else {
        fc_assert(false); // Huh?
      }
      break;
...
void dai_diplomacy_actions(struct ai_type *ait, struct player *pplayer)
{
  ...
  war_threshold =
      (MAX_AI_LOVE * (0.70 + aggr_sr / max_sr / 2.0)) - MAX_AI_LOVE;
  ...
  players_iterate_alive(aplayer)
  {
    ...
    if (adip->at_war_with_ally && adip->countdown == -1
        && !adip->is_allied_with_ally && !pplayers_at_war(pplayer, aplayer)
        && (player_diplstate_get(pplayer, aplayer)->type != DS_CEASEFIRE
            || fc_rand(5) < 1)) {
      DIPLO_LOG(ait, LOG_DEBUG, pplayer, aplayer,
                "plans war to help ally %s",
                player_name(adip->at_war_with_ally));
  ...
  players_iterate_alive(aplayer)
  {
    if (get_player_bonus(aplayer, EFT_NO_DIPLOMACY) <= 0
      ...
      if (is_barbarian(aplayer) || aplayer == pplayer
      ...
      case DS_ALLIANCE:
        ...
        if (adip->spam <= 0) {
          /* Count down patience toward AI player (one that can have spam >
           * 0) at the same speed as toward human players. */
          if (adip->ally_patience == 0) {
            dai_diplo_notify(
                aplayer,
                _("*%s (AI)* Greetings our most trustworthy "
                  "ally. We call upon you to destroy our enemy, %s."),
                player_name(pplayer), player_name(target));
            adip->ally_patience--;
      ...
      case DS_PEACE:
        ...
        if (adip->at_war_with_ally
            ...
            || dai_goldequiv_clause(ait, pplayer, aplayer, &clause, false,
                                    DS_ALLIANCE)
                   < 0) {
          break;
        }
    ...
    if (pplayer != violator) {
      ...
      if (victim == pplayer) {
        pplayer->ai_common.love[player_index(violator)] =
            MIN(pplayer->ai_common.love[player_index(violator)]
                    - MAX_AI_LOVE / 3,
                -1);
        // Scream for help!!
        players_iterate_alive(ally)
        {
          ...
          dai_diplo_notify(
              ...
              player_name(victim), player_name(violator));
        }
        players_iterate_alive_end;
      }
    }
