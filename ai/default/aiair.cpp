...
#define PROB_MULTIPLIER 100 // should unify with those in combat.c
  ...
  sortie_time = (utype_pays_mp_for_action_estimate(
                     action_by_number(ACTION_ATTACK), unit_type_get(punit),
                     unit_owner(punit),
  ...
  profit =
      kill_desire(victim_cost, unit_attack, unit_cost, victim_defence, 1)
                                               ...
                                               PFPath *path)
{
  ...
  pf_map_move_costs_iterate(pfm, ptile, move_cost, false)
  {
    ...
    if (!pvirtual) {
      pvirtual = unit_virtual_create(
          pplayer, player_city_by_number(pplayer, punit->homecity),
                             ...
                             bool allow_gold_upkeep)
{
  ...
  unit_type_iterate(punittype)
  {
    ...
    if (!allow_gold_upkeep
        && utype_upkeep_cost(punittype, pplayer, O_GOLD) > 0) {
      continue;
    }
