                                               ...
                                               bool allow_gold_upkeep)
{
  ...
  unit_type_iterate(ut)
  {
    struct unit_type_ai *utai =
        static_cast<unit_type_ai *>(utype_ai_data(ut, ait));
    int desire;
    ...
    desire = (ut->hp * ut->attack_strength * ut->firepower * ut->move_rate
              + ut->defense_strength)
             / MAX(UNITTYPE_COSTS(ut), 1);
    ...
    desire = amortize(desire, (utype_build_shield_cost(pcity, ut)
                                    ...
                                    struct adv_choice *choice)
{
  ...
  if (best > choice->want) {
    CITY_LOG(LOGLEVEL_HUNT, pcity, "pri missile w/ want " ADV_WANT_PRINTF,
             best);
    ...
    CITY_LOG(LOGLEVEL_HUNT, pcity,
             "not pri missile w/ want " ADV_WANT_PRINTF
             "(old want " ADV_WANT_PRINTF ")",
             best, choice->want);
  }
}
    ...
    if (unit_owner(missile) == pplayer
      ...
      pf_map_move_costs_iterate(pfm, ptile, move_cost, false)
      {
        ...
        unit_list_iterate(ptile->units, victim)
        {
          ...
          if (victim == target) {
            sucker = victim;
            UNIT_LOG(LOGLEVEL_HUNT, missile,
                      ...
                      struct unit *punit)
{
  ...
  if (original_target) {
    dai_hunter_juiciness(pplayer, punit, original_target, &original_threat,
                         &original_cost);
  }
